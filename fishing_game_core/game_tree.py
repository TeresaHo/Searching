...
class Node:
    ...
    def compute_and_get_children(self):
        ...
        if caught[current_player] is not None:
            # Next action is always up (action : 1) for the current player
            ...
            for act in range(5):
                new_state = self.compute_next_state(self.state, act, self.observations[self.depth])
    ...
    def compute_next_state(self, current_state, act, observations):
        ...
        # next_caught_fish =  [fish_number, fish_number]
        ...
        for i_player, fish_number in enumerate(pull_in_fishes):
            if fish_number is not None:
                ...
                # Remove fish
                new_state.remove_fish(fish_number) #delete fish_number from fish position
